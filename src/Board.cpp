...
Board::Board(unsigned int dimension)
    : dimension(dimension),
      spaces(std::vector<Piece *>(pow(dimension, 2), nullptr)), nb_w_pieces(0),
      nb_b_pieces(0), turn(WHITE) {}
...
void Board::handle_turn() { (turn == WHITE) ? turn = BLACK : turn = WHITE; }

// GETTERS

std::vector<Piece *> &Board::get_spaces() { return spaces; }

unsigned int Board::get_dimension() const { return dimension; }

Color Board::get_turn() const { return turn; }

Piece *Board::get_selected_piece() const { return selected_piece; }

unsigned int Board::get_nb_w_pieces() const { return nb_w_pieces; }
unsigned int Board::get_nb_b_pieces() const { return nb_b_pieces; }
...
void Board::set_space_style(const Color color, const size_t index,
                            int &nb_styles_push) {
  if (possible_spaces.find(index) != possible_spaces.end() &&
      possible_spaces[index]) {
    ...
    ImGui::PushStyleColor(ImGuiCol_Button, (color == WHITE)
                                               ? colors.WHITE_SPACE
                                               : colors.BLACK_SPACE);
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                          (color == WHITE) ? colors.WHITE_SPACE_HOVERED
                                           : colors.BLACK_SPACE_HOVERED);
    ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                          (color == WHITE) ? colors.WHITE_SPACE_ACTIVE
                                           : colors.BLACK_SPACE_ACTIVE);
    nb_styles_push += 3;
  }
}
...
void Board::set_color_scope(ImU32 &color_scope, const bool is_enemy) {
  color_scope =
      is_enemy ? colors.ENEMY_SCOPE_BORDER : colors.NORMAL_SCOPE_BORDER;
  if (ImGui::IsItemHovered()) {
    color_scope = is_enemy ? colors.ENEMY_SCOPE_BORDER_HOVERED
                           : colors.NORMAL_SCOPE_BORDER_HOVERED;
  }
  if (ImGui::IsItemActive()) {
    color_scope = is_enemy ? colors.ENEMY_SCOPE_BORDER_ACTIVE
                           : colors.NORMAL_SCOPE_BORDER_ACTIVE;
  }
}

void Board::set_border_thickness(ImVec2 &button_min, ImVec2 &button_max,
...
void Board::set_scope(const size_t index) {
  if (possible_spaces.find(index) != possible_spaces.end()) {
    ...
    ImGui::GetWindowDrawList()->AddRect(button_min, button_max, color_scope,
                                        0.0f, 0, border_thickness + 1.0f);
  }
}

void Board::set_piece_style(const Piece *piece_ptr, const char *&space_label) {
  if (piece_ptr) {
    (piece_ptr->get_color() == WHITE)
        ? ImGui::GetStyle().Colors[ImGuiCol_Text] = colors.WHITE_PIECE
        : ImGui::GetStyle().Colors[ImGuiCol_Text] = colors.BLACK_PIECE;
    space_label = piece_ptr->get_symbol();
  }
}

void Board::set_style(const size_t &index, Color color, int &nb_style_push,
                      const Piece *p, const char *&space_label) {
...
void Board::first_click_button(size_t index, const char *space_label,
  ...
  if (ImGui::Button(space_label, {SPACE_SIZE, SPACE_SIZE})) {
    ...
    history.nb_moves() > 0
        ? selected_piece->seach_positions_can_move(
              spaces, possible_spaces, dimension,
              {history.moves.back().piece,
               {history.moves.back().from_index,
                history.moves.back().to_index}})
        : selected_piece->seach_positions_can_move(
              spaces, possible_spaces, dimension, {nullptr, {0, 0}});
  }
}

void Board::classic_eat(size_t index, History &history) {
  if (spaces[index] &&
      spaces[index]->get_color() != selected_piece->get_color()) {
...
void Board::prise_en_passant(size_t index) {
  (selected_piece->get_color() == WHITE) ? spaces[index + dimension] = nullptr
                                         : spaces[index - dimension] = nullptr;
...
void Board::pawn_to_queen(Piece *&selected_piece, const size_t index) {
  if (std::string(selected_piece->get_symbol()) == "o") {
    if (index < dimension || index >= pow(dimension, 2) - dimension)
      selected_piece = new Queen(selected_piece->get_color(),
...
void Board::second_click_button(size_t index, const char *space_label,
  ...
  if (ImGui::Button(space_label, {SPACE_SIZE, SPACE_SIZE})) {
    if (possible_spaces.find(index) != possible_spaces.end()) {
      ...
      history.add_move({history.nb_moves() + 1, selected_piece,
                        selected_piece->get_position(), index});
...
void Board::create_button(size_t index, const char *space_label,
  ...
  if (spaces[index] && spaces[index]->get_color() == turn)
    ...
    (!selected_piece) ? no_action_button(space_label)
                      : second_click_button(index, space_label, history);
  set_scope(index);
}
...
void Board::update(History &history) { render(history); }
