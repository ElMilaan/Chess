...
public:
  ...
  static const inline std::vector<PieceType> PIECES_DISPOSITION{
      ROOK, KNIGHT, BISHOP, KING, QUEEN, BISHOP, KNIGHT, ROOK};
  const ColorUI colors{};

  Board(unsigned int dimension);

  std::vector<Piece *> &get_spaces();
  ...
  void set_border_thickness(ImVec2 &button_min, ImVec2 &button_max,
                            float border_thickness);
  void set_scope(const size_t index);

  void set_style(const size_t &index, Color color, int &nb_style_push,
