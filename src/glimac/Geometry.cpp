...
void Geometry::generateNormals(unsigned int meshIndex)
{
    ...
    for (auto j = 0u; j < m_MeshBuffer[meshIndex].m_nIndexCount; j += 3)
    {
        ...
        auto n = glm::cross(glm::normalize(m_VertexBuffer[i2].m_Position - m_VertexBuffer[i1].m_Position), glm::normalize(m_VertexBuffer[i3].m_Position - m_VertexBuffer[i1].m_Position));
