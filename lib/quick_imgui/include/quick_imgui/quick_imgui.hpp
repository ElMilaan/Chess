...
struct Callbacks {
    ...
    std::function<void(double, double)>     cursor_position_callback{[](double xpos, double ypos) {
    }}; // NOLINT(*-easily-swappable-parameters)
    std::function<void(double, double)>     scroll_callback{[](double xoffset, double yoffset) {
    }}; // NOLINT(*-easily-swappable-parameters)
    std::function<void(int, int)>           window_size_callback{[](int width, int height) {
    }}; // NOLINT(*-easily-swappable-parameters)
};
